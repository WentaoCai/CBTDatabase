...
expr<-fread("data/All.gene.fpkm2.txt",sep = "\t",header = T)
...
klncRNA_gtf<-fread("data/novel_lncRNA1.gtf",header=F)
head(klncRNA_gtf)
colnames(klncRNA_gtf)<-c("Chrom","type","start","end","strand","Transcript_id","Gene_id","transcript_biotype")
lncRNA_gene.expr<-fread("data/lncRNA.gene.expression.fpkm.txt",header=T)
lncRNA_trans.expr<-fread("data/lncRNA.transcript.expression.fpkm.txt",header=T)

editing_ann<-fread("data/All.AG.editing.VEP.useful7.txt",header=F)
